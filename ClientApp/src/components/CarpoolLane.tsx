...
export default function CarpoolLane({
  setSelectedStudents,
  selectedStudents,
  setDisabledStudents,
  setvalidRegistrationNumber,
  ...
  const [pickupcarForEdit, setpickupCarforEdit] = useState<
    PickupCar | undefined
  >();
  ...
  function AddLeftCarDisabledStudents(cars: PickupCar[]) {
    ...
    cars.forEach((car) => {
      if (car.hasLeft) array.push(...car.students);
    });
  ...
  function submitCar() {
    ...
    if (isEditCarEnabled && pickupcarForEdit != undefined) {
      ...
      var index: number = pickupCars.findIndex(
        (x) => x.id === pickupcarForEdit.id
      );
      axios
        .post<PickupCar>(baseURL + "PickupCar/UpdatePickupCar", car)
        .then((response) => {
          ...
          notyf.success(`Pickup car ${car.registrationNumber} updated Successfully`);
        });
    } else {
      axios
        .post<PickupCar>(baseURL + "PickupCar/CreatePickupCar", car)
        .then((response) => {
          setPickupCars([response.data, ...pickupCars]);
          resetCarpoolDashboardValues();
          notyf.success(`Pickup car ${response.data.registrationNumber} added Successfully`);
        });
    }
  }
  ...
  function handleInputchanges(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (value.length == 0) {
      ...
      pickupCars.findIndex((p) => p.registrationNumber == value) >= 0
    ) {
      setvalidRegistrationNumber(false);
      setSomeErrorFlag(true);
  ...
  function handleCarDeletion(car: PickupCar) {
    ...
    axios
      .post<PickupCar>(baseURL + "PickupCar/DeletePickupcar", car)
      .then((response) => {
        setPickupCars([
          ...pickupCars.slice(0, index),
          ...pickupCars.slice(index + 1),
        ]);
        resetCarpoolDashboardValues();
        window.scrollTo(0, 0);
        notyf.success(`Pickup car ${car.registrationNumber} deleted Successfully`);
      });
  }
  ...
  function handleClearCarpool() {
    axios
      .post<PickupCar>(baseURL + "PickupCar/ClearPickupCars")
      .then((response) => {
        setPickupCars([]);
        resetCarpoolDashboardValues();
        setDisabledStudents([]);
        setShowClearConfirmation(false);
        notyf.success(`All pickup cars have been deleted`);
      });
  }

  function handleResetCarpool() {
    axios
      .post<Array<PickupCar>>(baseURL + "PickupCar/ResetPickupCars")
      .then((response) => {
        setPickupCars(response.data);
        resetCarpoolDashboardValues();
        setDisabledStudents([]);
        setShowResetConfirmation(false);
        notyf.success(`Carpool lane has been reset!`);
      });
  }
  ...
  function handleCarNext(index: number) {
    ...
    array.unshift(car);
    setPickupCars(array);
    notyf.success(`Pickup car ${car.registrationNumber} marked next in lane`);
  }
  ...
  return (
    <Segment>
      <Segment className="ui">
        <Form>
          <Form.Input
            placeholder="Add New Car Registration number"
          ...
          <div
            ...
            id="title-error"
            role="alert"
            style={{ color: "indianred", display: isSomeError ? "" : "none" }}
          >
            {error}
          </div>
          <Button
            disabled={!validRegistrationNumber}
            style={{ display: isEditCarEnabled ? "none" : "" }}
          ...
          <Button
            style={{ display: !isEditCarEnabled ? "none" : "" }}
            className="ui basic negative right button"
            onClick={() => {
              resetCarpoolDashboardValues();
            }}
          >
      ...
      <Segment>
        <div style={{ overflow: "auto" }} className="ui cards">
          ...
          >
            ...
            <span
              style={{
                display: pickupCars.length == 0 ? "" : "none",
                fontWeight: "normal",
                fontSize: "12px",
                marginLeft: "10px",
              }}
            >
              {" "}
              (Please add cars to carpool){" "}
            </span>
            <Button
              disabled={pickupCars.length == 0}
          ...
          {pickupCars.map((carObj, carIndex) => (
            <CarItem
              handleCarNext={handleCarNext}
              key={carObj.id}
              index={carIndex}
              disableCar={carObj.hasLeft ? true : false}
              handleCarLeft={handleCarLeft}
              handleCarDeletion={handleCarDeletion}
              car={carObj}
              handleCarEdit={handleCarEdit}
              isCarLeftError={iscarLeftError}
            />
          ))}
        </div>
      </Segment>
