...
export default function ClassItem({
  ...
  function handleSelectedStudent(classid: number, studentid: number) {
    var studentIndex: number = schoolclass.students.findIndex(
      (s) => s.id === studentid
    );
    var SelectstudentIndex = selectedStudents?.findIndex(
    ...
    if (studentIndex != -1 && SelectstudentIndex == -1) {
      setSelectedStudents([
        ...selectedStudents,
        schoolclass.students[studentIndex],
      ]);
      console.log(selectedStudents);
    } else if (SelectstudentIndex != -1) {
      setSelectedStudents([
        ...selectedStudents.slice(0, SelectstudentIndex),
        ...selectedStudents.slice(SelectstudentIndex + 1),
      ]);
    }
  }

  function getdisabledProperty(disabledStudents: Student[], currentId: number) {
    if (!validRegistrationNumber) return true;
    return disabledStudents.findIndex((student) => student.id == currentId) >= 0
      ? true
      : false;
  }

  function getcheckedProperty(selectedStudents: Student[], currentId: number) {
    return selectedStudents.findIndex((student) => student.id == currentId) >= 0
      ? true
      : false;
  }
      ...
      <Header>
        ...
        <span className="ui" style={{ float: "right", fontSize: "12px" }}>
          ...
          <span style={{ marginLeft: "5px" }}>
            {" "}
            Students Remaining -{" "}
            {schoolclass.students.length - getStudentsLeft()}
          </span>
        </span>
      </Header>
      <List horizontal>
        {schoolclass.students.map((studentobj: Student) => (
          <ListItem key={studentobj.id}>
            <>
              <input
                type="checkbox"
                disabled={getdisabledProperty(disabledStudents, studentobj.id)}
                checked={getcheckedProperty(selectedStudents, studentobj.id)}
                onChange={() => {}}
                style={{ marginRight: 10 }}
                onClick={() =>
                  handleSelectedStudent(schoolclass.id, studentobj.id)
                }
                value={studentobj.id}
              ></input>
              <span
                style={{
                  opacity: getdisabledProperty(disabledStudents, studentobj.id)
                    ? 0.45
                    : "",
                  color: getdisabledProperty(disabledStudents, studentobj.id)
                    ? "rgba(40, 40, 40, 0.3)"
                    : "",
                }}
              >
