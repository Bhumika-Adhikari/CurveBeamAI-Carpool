...
export default function CarItem({
  ...
  return (
    <div
      className="card"
      style={{
        opacity: disableCar ? 0.45 : "",
        color: disableCar ? "rgba(40,40,40,.3)" : "",
      }}
    >
      <div className="content">
        <div className="header">
          ...
          <div
            ...
            style={{
              display: isCarLeftError && index == 0 ? "" : "none",
              fontSize: "10px",
              marginTop: "10px",
            }}
          >
            <div className="header">
              Car cannot be marked left, Please remove student/s who has already
              left.
            </div>
            <p></p>
          </div>
        ...
        <div className="description">
          {car.students.map((student: Student) => (
            <List key={student.id}>{student.studentName}</List>
          ))}
        </div>
      </div>
      <div
        className="extra content"
        style={{ display: disableCar ? "none" : "block" }}
      >
        <span className="left floated">
          <button
            style={{ display: index == 0 ? "" : "none" }}
            className="ui positive   button"
            onClick={() => handleCarLeft(car)}
          >
            Mark Car Left
          </button>
          <button
